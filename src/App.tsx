    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
        <section className="mb-10 text-center md:text-left md:flex md:items-end md:justify-between gap-4">
          <div>
            <h2 className="text-xl sm:text-3xl font-extrabold text-slate-900 mb-2">新鮮手作義大利麵</h2>
            <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm text-slate-500">
              <span className="flex items-center gap-1"><Clock className="w-4 h-4" /> 預計取餐：20-30 分鐘</span>
              <span className="flex items-center gap-1"><MapPin className="w-4 h-4" /> 台北市大安區忠孝東路</span>
            </div>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredMenu.map(item => (
            <div key={item.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden group hover:shadow-md transition-shadow">
              <div className="relative h-40 sm:h-48 overflow-hidden">
                <img 
                  src={item.image} 
                  alt={item.name} 
                  className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                  onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1473093226795-af9932fe5856?auto=format&fit=crop&q=80&w=400'; }}
                />
